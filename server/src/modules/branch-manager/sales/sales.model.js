...
export async function listSales(tenantId, filters = {}) {
  ...
  if (filters.q) {
    ...
    query += ` AND (s.sale_number ILIKE $${params.length} OR s.id::text ILIKE $${params.length})`
  }

  if (filters.date) {
    params.push(filters.date)
    query += ` AND s.sold_at::date = $${params.length}::date`
  }

  // Filter by product category (subquery on sale items)
  if (filters.category_id) {
    ...
    query += ` AND s.id IN (
      ...
      WHERE p2.category_id = $${params.length}
    )`
  }
